...
router.get('/index', (req,res)=> {
    Cours.find()
    .then((result) => {
      res.render("cours/index", { courses: result, message: req.flash('info') });
    })
    .catch((err) => {
      console.log(err)
    })
...
[   check('title').isLength({min: 5}).withMessage('Titel should be more than 5 char'),
    check('content').isLength({min: 50}).withMessage('Lesson should be more than 50 char'),
    check('level').isLength({min: 6}).withMessage('level should be more than 5 char')
], 
(req,res)=> {
